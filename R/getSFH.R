getSFHfull = function(file_sting='mocksurvey.hdf5', path_shark='.', cores_per_subvolume=4, verbose=TRUE){
  ...
  outSFH = list(SFRbulge_d=output[,1:Ntime+Ntime*0], SFRbulge_m=output[,1:Ntime+Ntime*1],
                SFRdisk=output[,1:Ntime+Ntime*2], Zbulge_d=output[,1:Ntime+Ntime*3],
                Zbulge_m=output[,1:Ntime+Ntime*4], Zdisk=output[,1:Ntime+Ntime*5])
...
getSFHsing = function(id_galaxy_sam, snapshot=NULL, subvolume=NULL, path_shark='.'){
  ...
  outSFH=list(id_galaxy_sam=id_galaxy_sam, keep=keep, SFRbulge_d=SFRbulge_d,
              SFRbulge_m=SFRbulge_m, SFRdisk=SFRdisk, Zbulge_d=Zbulge_d,
              Zbulge_m=Zbulge_m, Zdisk=Zdisk)
  class(outSFH) = 'Viperfish-SFH'
  invisible(outSFH)
}
