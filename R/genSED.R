genSED=function(SFRbulge_d, SFRbulge_m, SFRdisk, redshift=0.1, time=NULL, tau_birth=1, tau_screen=0.3, pow_birth=-0.7, pow_screen=-0.7, alpha_SF=1, AGNfrac=0, Zbulge_d=5, Zbulge_m=5, Zdisk=5, ab_nodust=TRUE, ap_nodust=TRUE, ab_dust=TRUE, ap_dust=TRUE, emitdust=TRUE, unimax=13.8e9, speclib=NULL, Dale=NULL, filtout=NULL, H0=67.8, sparse=1, intSFR=TRUE){
  ...
  if(ap_nodust){
    bulge_d_nodust=SFHfunc(SFRbulge_dfunc, tau_birth=0, tau_screen=0, speclib=speclib, z=redshift, Z=Z[[1]], outtype=NULL, unimax=unimax, intSFR=intSFR)
    bulge_m_nodust=SFHfunc(SFRbulge_mfunc, tau_birth=0, tau_screen=0, speclib=speclib, z=redshift, Z=Z[[2]], outtype=NULL, unimax=unimax, intSFR=intSFR)
    disk_nodust=SFHfunc(SFRdiskfunc, tau_birth=0, tau_screen=0, speclib=speclib, z=redshift, Z=Z[[3]], outtype=NULL, unimax=unimax, intSFR=intSFR)
  }else if(ab_nodust | emitdust){
    bulge_d_nodust=SFHfunc(SFRbulge_dfunc, tau_birth=0, tau_screen=0, speclib=speclib, z=0, Z=Z[[1]], outtype=NULL, intSFR=intSFR)
    bulge_m_nodust=SFHfunc(SFRbulge_mfunc, tau_birth=0, tau_screen=0, speclib=speclib, z=0, Z=Z[[2]], outtype=NULL, intSFR=intSFR)
    disk_nodust=SFHfunc(SFRdiskfunc, tau_birth=0, tau_screen=0, speclib=speclib, z=0, Z=Z[[3]], outtype=NULL, intSFR=intSFR)
  }

  if(ap_dust){
    bulge_d_dust=SFHfunc(SFRbulge_dfunc, tau_birth=tau_birth[1], tau_screen=tau_screen[1], pow_birth=pow_birth[1], pow_screen=pow_screen[1], speclib=speclib, z=redshift, Z=Z[[1]], outtype=NULL, unimax=unimax, intSFR=intSFR)
    bulge_m_dust=SFHfunc(SFRbulge_mfunc, tau_birth=tau_birth[2], tau_screen=tau_screen[2], pow_birth=pow_birth[2], pow_screen=pow_screen[2], speclib=speclib, z=redshift, Z=Z[[2]], outtype=NULL, unimax=unimax, intSFR=intSFR)
    disk_dust=SFHfunc(SFRdiskfunc, tau_birth=tau_birth[3], tau_screen=tau_screen[3], pow_birth=pow_birth[3], pow_screen=pow_screen[3], speclib=speclib, z=redshift, Z=Z[[3]], outtype=NULL, unimax=unimax, intSFR=intSFR)
  }else if(ab_dust){
    bulge_d_dust=SFHfunc(SFRbulge_dfunc, tau_birth=tau_birth[1], tau_screen=tau_screen[1], pow_birth=pow_birth[1], pow_screen=pow_screen[1], speclib=speclib, z=0, Z=Z[[1]], outtype=NULL, intSFR=intSFR)
    bulge_m_dust=SFHfunc(SFRbulge_mfunc, tau_birth=tau_birth[2], tau_screen=tau_screen[2], pow_birth=pow_birth[2], pow_screen=pow_screen[2], speclib=speclib, z=0, Z=Z[[2]], outtype=NULL, intSFR=intSFR)
    disk_dust=SFHfunc(SFRdiskfunc, tau_birth=tau_birth[3], tau_screen=tau_screen[3], pow_birth=pow_birth[3], pow_screen=pow_screen[3], speclib=speclib, z=0, Z=Z[[3]], outtype=NULL, intSFR=intSFR)
  }
  ...
  if((ab_dust | ap_dust) & emitdust & !is.null(Dale)){
    ...
    dustout_b_d=dustmass(speclib$Wave, bulge_d_nodust$lum, bulge_d_dust$lum, Dale$Wave, Dale_interp_b_d[,2])
    dustout_b_m=dustmass(speclib$Wave, bulge_m_nodust$lum, bulge_m_dust$lum, Dale$Wave, Dale_interp_b_m[,2])
    dustout_d=dustmass(speclib$Wave, disk_nodust$lum, disk_dust$lum, Dale$Wave, Dale_interp_d[,2])
    ...
    if(ab_dust){
      ...
      bulge_d_dust$lum=addspec(Dale$Wave, Dale_interp_b_d[,2]*dustout_b_d[1], lumwave, bulge_d_dust$lum)[,2]
      bulge_m_dust$lum=addspec(Dale$Wave, Dale_interp_b_m[,2]*dustout_b_m[1], lumwave, bulge_m_dust$lum)[,2]
      disk_dust$lum=addspec(Dale$Wave, Dale_interp_d[,2]*dustout_d[1], lumwave, disk_dust$lum)[,2]
